...
public class KasPengeluaran extends javax.swing.JFrame {

    private Connection conn;
    
    public KasPengeluaran() {
        initComponents();
    ...
    private void getData() {
        ...
        try{
            String sql = "SELECT * FROM kas_pengeluaran";
            ...
            while (rs.next()) {
                ...
                int jumlahPengeluaran = rs.getInt("Jumlah_Pengeluaran");

                Object[] rowData = {no, uraian, tanggal, jumlahPengeluaran};
            ...
            Logger.getLogger(KasPengeluaran.class.getName()).log(Level.SEVERE, null, e);
        }
    }
...
private void insertData(String uraian, Date tanggal, int jumlahPengeluaran) {
    try {
        String sql = "INSERT INTO kas_pengeluaran (uraian, tanggal, jumlah_Pengeluaran) VALUES (?, ?, ?)";
        PreparedStatement st = conn.prepareStatement(sql);
        st.setString(1, uraian);
        st.setDate(2, new java.sql.Date(tanggal.getTime()));
        st.setInt(3, jumlahPengeluaran);
...
private void updateData(int no, String uraian, Date tanggal, int jumlahPengeluaran) {
    try {
        String sql = "UPDATE kas_pengeluaran SET uraian=?, tanggal=?, jumlah_Pengeluaran=? WHERE no=?";
        PreparedStatement st = conn.prepareStatement(sql);
        st.setString(1, uraian);
        st.setDate(2, new java.sql.Date(tanggal.getTime()));
        st.setInt(3, jumlahPengeluaran);
...
private void deleteData(int no) {
    try {
        String sql = "DELETE FROM kas_pengeluaran WHERE no=?";
        PreparedStatement st = conn.prepareStatement(sql);
        st.setInt(1, no);
...
// ...
    ...
    private void btn_tambahMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btn_tambahMouseClicked
        ...
        int jumlahPengeluaran = Integer.parseInt(t_jumlah.getText());

        if (uraian.isEmpty() || tanggal == null || jumlahPengeluaran == -1) {
            JOptionPane.showMessageDialog(this, "Isi Semua Data", "Validasi", JOptionPane.ERROR_MESSAGE);
            return;
        }

        insertData(uraian, tanggal, jumlahPengeluaran);
    ...
    if (selectedRow != -1) {
        int no = Integer.parseInt(tbl_data.getValueAt(selectedRow, 0).toString());
        String uraian = t_uraian.getText();
        Date tanggal = t_tanggal.getDate();
        int jumlahPengeluaran = Integer.parseInt(t_jumlah.getText());
        ...
        updateData(no, uraian, tanggal, jumlahPengeluaran);
    }
    ...
    private void tbl_dataMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_dataMouseClicked
        ...
        if (selectedRow != -1) {
            ...
            int jumlahPengeluaran = Integer.parseInt(tbl_data.getValueAt(selectedRow, 3).toString());

            t_uraian.setText(uraian);
            t_tanggal.setDate(tanggal);
            t_jumlah.setText(String.valueOf(jumlahPengeluaran));
        }
    ...
    try {
        String sql = "SELECT * FROM kas_pengeluaran WHERE Uraian LIKE ? OR Tanggal LIKE ? OR Jumlah LIKE ?";
        PreparedStatement st = conn.prepareStatement(sql);
        st.setString(1, "%" + cari + "%");
        st.setString(2, "%" + cari + "%");
        st.setString(3, "%" + cari + "%");
    ...
    private void createTable() {
        try {
            String createTableSQL = "CREATE TABLE IF NOT EXISTS kas_pengeluaran ("
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(KasPengeluaran.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                KasPengeluaran kasPengeluaran = new KasPengeluaran();
                kasPengeluaran.createTable();  // Call createTable when the application starts
                kasPengeluaran.setVisible(true);
            }
        });
    }
